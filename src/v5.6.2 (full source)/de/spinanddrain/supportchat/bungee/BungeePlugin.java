...
public class BungeePlugin extends Plugin implements Server {
	...
	public static Object get(int i, String path) {
		switch (i) {
		case 0:
			return BungeePlugin.provider.getConfig().getAdapter().cfg.get(path);
		case 1:
			return BungeePlugin.provider.getMessages().getParser().getByKey(path);
		case 2:
			return BungeePlugin.provider.getReasons().getAdapter().cfg.get(path);
		case 3:
			return BungeePlugin.provider.getAddons().getAdapter().cfg.get(path);
		default:
			return null;
		}
	}
